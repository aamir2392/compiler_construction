...
namespace ProjectCompiler
{
    ...
    public partial class Form1 : Form
    {
        ...
        List<String> iList = new List<String>();        // identifiers
        ...
        public void createSymbols()
        {
            for(int i = 0; i < 17; i++)
            {
                Symbols identifier = new Symbols();
                String op = "";
                switch(i)
                {
                    case 0:
                    ...
                    case 5:
                        identifier.symbol = "(";
                        identifier.name = "open bracket";
                        break;
                    case 6:
                        identifier.symbol = ")";
                        identifier.name = "close bracket";
                        break;
                    case 7:
                        identifier.symbol = "{";
                        identifier.name = "open curly bracket";
                        break;
                    case 8:
                        identifier.symbol = "}";
                        identifier.name = "close curly bracket";
                        break;
                    case 9:
                        identifier.symbol = ",";
                        identifier.name = "comma";
                        break;
                    case 10:
                        identifier.symbol = ";";
                    ...
                    case 13:
                        identifier.symbol = "<";
                        identifier.name = "less than";
                        break;
                    case 14:
                        identifier.symbol = ">";
                        identifier.name = "greater than";
                        break;
                    case 15:
                        identifier.symbol = "=";
                ...
                try
                {
                    if (isIdentifier(code[i]))
                    {
                        ...
                        if (isVariable(code[i + 1]))
                        {
                            ...
                            {
                                // well
                                identifier.name = code[i + 1];
                                identifier.value = "null";
                                memoryList.Add(identifier);
                            }
                            else
                            {
                                if (code[i + 2] == "=")
        ...
        {
            MemoryCalculating identifier = new MemoryCalculating();
            if (i == 0 || inCondition == 1)
            {
                try
                {
                    if (isVariable(code[i]))
                    {
                        if (code[i + 1] == "=")
                        {
                            if (isVariable(code[i + 2]) || isNumber(code[i + 2]))
                            {
                                if (code[i + 3] == ";")
                                {
                                    identifier.name = code[i];
                                    identifier.statement.Add(code[i + 2]);
                                    calcList.Add(identifier);
                                    tempCalcList.Add(identifier);
                                    // then well
                                }
                                else
                                {
                                        ...
                                        if (isOperator(code[i + 3]))                // || isOperator(code[i + 3][0]) && code[i + 3] == "="
                                        {
                                            ...
                                            if (isVariable(code[i + 4]) || isNumber(code[i + 4]))
                                            {
                                                if (code[i + 5] == ";")
                                                {
                                                    if (j == i)
                                                    {
                                                        identifier.name = code[j];
                                                        identifier.statement.Add(code[i + 2]);
                                                        identifier.statement.Add(code[i + 3]);
                                                        identifier.statement.Add(code[i + 4]);
                                                        calcList.Add(identifier);
                                                        tempCalcList.Add(identifier);
                                                    }
                                                    else
